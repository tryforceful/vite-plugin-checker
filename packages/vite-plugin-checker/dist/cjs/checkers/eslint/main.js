...
const createDiagnostic = (pluginConfig) => {
  ...
  return {
    ...
    async configureServer({ root }) {
      ...
      const dispatchDiagnostics = () => {
        ...
        if (terminal) {
          ...
          ;(0, import_logger.consoleLog)(
            (0, import_logger.composeCheckerSummary)('ESLint', errorCount, warningCount)
          )
        }
        if (overlay) {
      ...
      const handleFileChange = async (filePath, type) => {
        ...
        if (type === 'unlink') {
          ...
          const newDiagnostics = diagnosticsOfChangedFile
            .map((d) => (0, import_logger.normalizeEslintDiagnostic)(d))
            .flat(1)
      ...
      manager.initWith(
        diagnostics.map((p) => (0, import_logger.normalizeEslintDiagnostic)(p)).flat(1)
      )
      dispatchDiagnostics()
