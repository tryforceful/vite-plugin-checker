...
const createDiagnostic = (pluginConfig) => {
  ...
  return {
    ...
    async configureServer({ root }) {
      ...
      const translatedOptions = await (0, import_options.translateOptions)(
        ...
        if (terminal) {
          diagnostics2.forEach((d) => {
            ;(0, import_logger.consoleLog)(
              (0, import_logger.diagnosticToTerminalLog)(d, 'Stylelint')
            )
          })
          ...
          ;(0, import_logger.consoleLog)(
            (0, import_logger.composeCheckerSummary)('Stylelint', errorCount, warningCount)
          )
        }
        if (overlay) {
          ;(_a2 = import_worker_threads.parentPort) == null
            ? void 0
            : _a2.postMessage({
      ...
      const handleFileChange = async (filePath, type) => {
        ...
        if (type === 'unlink') {
          ...
          const newDiagnostics = diagnosticsOfChangedFile
            .map((d) => (0, import_logger.normalizeStylelintDiagnostic)(d))
            .flat(1)
      ...
      manager.initWith(
        diagnostics.map((p) => (0, import_logger.normalizeStylelintDiagnostic)(p)).flat(1)
      )
      dispatchDiagnostics()
