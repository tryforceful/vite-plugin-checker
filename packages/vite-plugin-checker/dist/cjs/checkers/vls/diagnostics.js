...
async function diagnostics(
  workspace,
  logLevel,
  options = { watch: false, verbose: false, config: null }
) {
  var _a
  if (options.verbose) {
    console.log('====================================')
  ...
  if (!options.watch && typeof result === 'object' && result !== null) {
    ...
    ;(_a = options == null ? void 0 : options.onDispatchDiagnosticsSummary) == null
      ? void 0
      : _a.call(options, initialErrorCount, initialWarningCount)
...
async function prepareClientConnection(workspaceUri, severity, options) {
  ...
  serverConnection.sendDiagnostics = async (publishDiagnostics) => {
    ...
    const nextDiagnosticInFile = await (0, import_logger.normalizePublishDiagnosticParams)(
      publishDiagnostics
    )
    ...
    if (initialVueFilesTick >= initialVueFilesCount) {
      ...
      ;(_b = options.onDispatchDiagnosticsSummary) == null
        ? void 0
        : _b.call(options, errorCount, warningCount)
    }
  }
      ...
      try {
        ...
        if (diagnostics2.length > 0) {
          logChunk +=
            ...
            diagnostics2
              .map((d) =>
                (0, import_logger.diagnosticToTerminalLog)(
